...
export function renderSearch(container, user, profile) {
  ...
  function closeModal() {
    modalEl.innerHTML = ''
    saveTarget = null
    resultsEl.innerHTML = '' // Limpiar resultados también
  }

  // ✅ ACTUALIZADO: Ahora guarda displayName del militante
  async function doSave(withWA) {
    ...
    try {
      // ✅ AGREGAR: Guardar displayName del militante
      const militanteName = user.displayName || user.email || 'Desconocido'
      await saveRecord(user.uid, saveTarget, telefono, nota, transporte, false, militanteName)
