...
async function loadAndRender(container) {
  try {
    ...
    onSnapshot(collection(db, 'choferes'), (choferesRealtime) => {
      ...
      // Se actualizará cuando se ejecute renderChoferes en el listener de votos
    })
    ...
    onSnapshot(
      ...
      votosSnap => {
        ...
        getDocs(collection(db, 'savedRecords')).then(votantesSnap => {
          ...
          renderChoferes(choferes, allRecords, allVotos, db, doc, deleteDoc)
        })
      }
    )
...
function renderChoferes(choferes, allRecords, allVotos, db, doc, deleteDoc) {
  let html = ''
...
function mostrarDetalle(nombre, registros, votos, choferes, db, setDoc, doc) {
  ...
  // Separar por estado
  const votaron = votos.filter(v => v.estado === 'votó')
  ...
  // SECCIÓN 1: YA VOTARON
  if (votaron.length > 0) {
    html += '<div style="margin-bottom: 24px;"><h3 style="background: #2e7d32; color: white; padding: 12px; border-radius: 4px; margin: 0 0 12px 0;">🟢 YA VOTARON (' + votaron.length + ')</h3>'
    votaron.forEach(v => {
  ...
  if (enCamino.length > 0) {
    ...
    enCamino.forEach(v => {
      ...
      if (r) {
        ...
        html += '<div style="font-weight: 600;">' + (v.nombre || r.nombre) + '</div>'
        html += '<div style="font-size: 0.8rem; color: #333;">CI: ' + v.cedula + ' | Local: ' + (r.local || 'N/A') + ' | Mesa: ' + (r.mesa || 'N/A') + ' | 📱 ' + (v.telefono || 'N/A') + '</div>'
        if (v.choferAsignado) {
          html += '<div style="font-size: 0.8rem; color: #e65100; margin-top: 4px;">🚗 Chofer: ' + v.choferAsignado + ' | ⏰ ' + (v.horarioBusqueda || 'N/A') + '</div>'
        }
        html += '</div>'
      }
    })
  ...
  // SECCIÓN 3: FALTANTES (EDITABLE)
  if (faltantes.length > 0) {
    html += '<div><h3 style="background: #c41e3a; color: white; padding: 12px; border-radius: 4px; margin: 0 0 12px 0;">🔴 FALTANTES (' + faltantes.length + ') - EDITABLE</h3>'
    faltantes.forEach(r => {
      ...
      html += '<button class="btn-estado" data-cedula="' + r.cedula + '" data-estado="no_votó" style="background: #ffcdd2; color: #c41e3a; border: none; padding: 6px; border-radius: 4px; cursor: pointer; font-size: 0.8rem; font-weight: 600;">🔴 No votó</button>'
      html += '<button class="btn-estado" data-cedula="' + r.cedula + '" data-estado="en_camino" style="background: #ffe0b2; color: #e65100; border: none; padding: 6px; border-radius: 4px; cursor: pointer; font-size: 0.8rem; font-weight: 600;">🟡 En camino</button>'
      html += '<button class="btn-estado" data-cedula="' + r.cedula + '" data-estado="votó" style="background: #c8e6c9; color: #2e7d32; border: none; padding: 6px; border-radius: 4px; cursor: pointer; font-size: 0.8rem; font-weight: 600;">🟢 Votó</button>'
      html += '</div>'

      // Chofer (si es en camino)
      ...
      html += '<div><label style="font-size: 0.8rem; font-weight: 600;">Dirección:</label>'
      html += '<input type="text" class="direccion-' + r.cedula + '" data-cedula="' + r.cedula + '" placeholder="Calle y número" style="width: 100%; padding: 6px; border: 1px solid #ddd; border-radius: 4px; font-size: 0.8rem;"></div>'
      html += '</div>'

      html += '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 8px;">'
      html += '<div><label style="font-size: 0.8rem; font-weight: 600;">Horario:</label>'
      html += '<input type="time" class="horario-' + r.cedula + '" data-cedula="' + r.cedula + '" style="width: 100%; padding: 6px; border: 1px solid #ddd; border-radius: 4px; font-size: 0.8rem;"></div>'
      
      html += '<div style="display: flex; gap: 4px; align-items: flex-end;">'
      html += '<button class="btn-guardar-cambios" data-cedula="' + r.cedula + '" style="flex: 1; background: #1976d2; color: white; border: none; padding: 6px; border-radius: 4px; cursor: pointer; font-size: 0.8rem; font-weight: 600;">💾 Guardar</button>'
      if (r.telefono) {
        html += '<a href="' + waLink + '" target="_blank" style="flex: 1; background: #25d366; color: white; padding: 6px; border-radius: 4px; text-decoration: none; font-size: 0.8rem; font-weight: 600; text-align: center;">💬 WA</a>'
      }
      html += '</div></div>'
  ...
  document.querySelectorAll('.btn-estado').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const votoExistente = votos.find(v => v.cedula === cedula)
      
      if (votoExistente) {
        // Actualizar voto existente
        setDoc(doc(db, 'dia_d_votos', votoExistente.id), { 
          estado: estado,
        ...
        // Crear nuevo voto
        const firebaseImport = import('firebase/firestore').then(mod => {
          const { addDoc, collection, serverTimestamp } = mod
          addDoc(collection(db, 'dia_d_votos'), {
            cedula: cedula,
            nombre: document.querySelector('[data-cedula="' + cedula + '"]').closest('div').querySelector('[style*="font-weight"]').textContent,
            estado: estado,
            militante_id: 'samy_admin',
            timestamp: new Date(),
            ultimoCambio: new Date()
          }).then(() => alert('✅ Registrado')).catch(err => alert('Error: ' + err.message))
        })
      }
    })
  })
  ...
  document.querySelectorAll('.btn-guardar-cambios').forEach(btn => {
    btn.addEventListener('click', async () => {
      const cedula = btn.dataset.cedula
      const chofer = document.querySelector('.chofer-select-' + cedula).value
      const direccion = document.querySelector('.direccion-' + cedula).value
      const horario = document.querySelector('.horario-' + cedula).value

      const votoExistente = votos.find(v => v.cedula === cedula)
      if (votoExistente) {
        try {
          await setDoc(doc(db, 'dia_d_votos', votoExistente.id), {
